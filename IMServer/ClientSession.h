...
public:
	...
	void Send(const TcpConnectionPtr& conn, BR::BinaryWriter& writer)
	{
		...
		writer.WriteData<int>(len+6);
		writer.WriteData(htonl(len));//包的长度，使用网络字节序进行传输，确保在不同平台之间的兼容性
		writer.WriteData(htons(0));//包的类型，暂时设置为0，可以根据需要定义不同的类型来区分不同的消息
		out = writer.toString() + out;
		if (conn != NULL)
		{
			BR::BinaryReader::dump(out);
