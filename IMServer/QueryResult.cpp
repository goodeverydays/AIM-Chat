...
QueryResult::QueryResult(MYSQL_RES* result, uint32_t rowcount, uint32_t cloumncount)
	:m_result(NULL), m_rowcount(0), m_cloumncount(0)
{
	m_CurrentRow.resize(m_rowcount);
	...
	for (uint32_t i = 0; i < m_cloumncount; i++)
	{
		...
		m_CurrentRow[i].SetName(fields[i].name);//设置字段的名称，方便通过字段名称获取字段值
	}
}
...
bool QueryResult::NextRow()
{
	...
	uint32_t* pFiledLength = (uint32_t*)mysql_fetch_lengths(m_result);//获取当前行中每个字段的长度信息，返回一个指向长度数组的指针，数组中的每个元素对应一个字段的长度
	for (uint32_t i = 0; i < m_cloumncount; i++)
	{
		if (row[i] == NULL)
		{
